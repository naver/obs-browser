...
QCefWidgetImpl::QCefWidgetImpl(QWidget *parent, const std::string &url,
			       ...
			       const QColor &initBkgColor,
			       const std::string &css, bool showAtLoadEnded)
	: QWidget(parent),
	  m_browser(),
	  m_url(url),
	  m_script(script),
...
void QCefWidgetImpl::init()
{
	executeOnCef([this,
		...
		windowInfo.SetAsChild(handle, CefRect(0, 0, size.width(),
						      size.height()));
...
bool QCefWidgetImpl::nativeEvent(const QByteArray &eventType, void *message,
				 qintptr *result)
{
	MSG *msg = (MSG *)message;
	switch (msg->message) {
...
void QCefWidgetImpl::sendMsg(const std::wstring &type, const std::wstring &msg)
{
	...
	executeOnCef([browser = m_browser, type = std::wstring(type.c_str()),
...
//PRISM/Zhangdewen/20230117/#/libbrowser
void QCefWidgetImpl::executeOnCef(
	...
	: QCefWidget(parent)
{
	...
	m_impl = new QCefWidgetImpl(this, url, script, rqc, headers,
				    allowPopups, popup, initBkgColor, css,
				    showAtLoadEnded);
	cef_widgets_add(m_impl, locked);

	connect(m_impl, &QCefWidgetImpl::titleChanged, this,
		&QCefWidgetInternal::titleChanged);
	connect(m_impl, &QCefWidgetImpl::urlChanged, this,
		&QCefWidgetInternal::urlChanged);
	connect(m_impl, &QCefWidgetImpl::loadEnded, this,
		&QCefWidgetInternal::loadEnded);
	connect(m_impl, &QCefWidgetImpl::msgRecevied, this,
		&QCefWidgetInternal::msgRecevied);
...
//PRISM/Zhangdewen/20230117/#/libbrowser
QCefWidgetInternal::QCefWidgetInternal(QCefBrowserPopupDialog *parent,
				       QCefWidgetImpl *checkImpl, bool locked)
	: QCefWidgetInternal(parent, checkImpl->m_url, checkImpl->m_rqc,
			     checkImpl->m_script, checkImpl->m_headers,
...
QCefBrowserPopupDialog::QCefBrowserPopupDialog(QCefWidgetImpl *checkImpl,
					       QWidget *parent, bool locked)
	: QDialog(parent), m_checkImpl(checkImpl)
{
...
//PRISM/Zhangdewen/20210330/#/optimization, maybe crash
QCefBrowserPopupDialog *
QCefBrowserPopupDialog::create(CefWindowHandle &handle,
			       QCefWidgetImpl *checkImpl, QWidget *parent,
			       bool locked)
{
	if (QThread::currentThread() == qApp->thread()) {
		QCefBrowserPopupDialog *dialog =
			new QCefBrowserPopupDialog(checkImpl, parent, locked);
	...
	QMetaObject::invokeMethod(
		qApp,
		[&dialog, &handle, checkImpl, parent, locked]() {
			dialog = create(handle, checkImpl, parent, locked);
		},
			    ...
			    const std::string &css, bool showAtLoadEnded)
{
	...
	return new QCefWidgetInternal(parent, url, cmi ? cmi->rc : nullptr,
				      script, headers, allowPopups, true,
				      nullptr, false, initBkgColor, css,
				      showAtLoadEnded);
}
