...
public:
	inline QCefBrowserClient(QCefWidgetInternal *widget_, const std::string &script_, bool allowAllPopups_,
				 //PRISM/Zhangdewen/20230308/#/libbrowser
				 const std::map<std::string, std::string> &headers_, const std::string &css_)
		: widget(widget_),
		  script(script_),
...
class PLSCefBrowserPopupDialog : public QDialog {
	Q_OBJECT
public:
	PLSCefBrowserPopupDialog(QCefWidgetInternal *parent, const std::string &_url);
...
public:
	inline PLSPopupClientHandlerStd(PLSCefBrowserPopupDialog *dialog, const std::string &script_,
					bool allowAllPopups_, const std::map<std::string, std::string> &headers_,
					const std::string &css_)
		: QCefBrowserClient(dialog->internal(), script_, allowAllPopups_, headers_, css_),
		  m_dialog(dialog)
	{
	}
