...
struct PLSQCefCookieManager : public QCefCookieManager {
	...
	virtual void ReadCookies(const std::string &site, const std::function<void(const std::list<Cookie> &)> &cookies,
				 bool isOnlyHttp) = 0;
	virtual void ReadAllCookies(const std::function<void(const std::list<Cookie> &)> &cookies) = 0;
	virtual bool SetCookie(const std::string &url, const std::string &name, const std::string &value,
			       const std::string &domain, const std::string &path, bool isOnlyHttp) = 0;
};
...
public:
	//PRISM/Zhangdewen/20230117/#/libbrowser
	virtual void sendMsg(const std::wstring &type, const std::wstring &msg) = 0;
...
struct PLSQCef : public QCef {
	...
	create_widget(QWidget *parent, const std::string &url, const std::string &script,
		      ...
		      const std::string &css = std::string(), bool showAtLoadEnded = false) = 0;
};
