...
//-------------------------------------------------------
void InteractionViewWin::RegisterClassName()
{
...
void InteractionViewWin::UnregisterClassName()
{
	::UnregisterClass(INTERACTION_CLASS_VIEW, GetModuleHandle(NULL));
}

LRESULT CALLBACK InteractionViewWin::InteractionViewProc(HWND hWnd,
							 UINT message,
							 WPARAM wParam,
							 LPARAM lParam)
{
	...
	InteractionViewWin *self = GetUserDataPtr<InteractionViewWin *>(hWnd);
...
//------------------------------------------------------------------------
InteractionViewWin::InteractionViewWin(BrowserInteractionMain &interaction_main)
	: InteractionView(interaction_main)
{
}

InteractionViewWin::~InteractionViewWin()
{
	assert(!::IsWindow(hwnd_));
}

void InteractionViewWin::Create(WindowHandle hp)
{
	assert(!::IsWindow(hwnd_));
	...
	ime_handler_ = nullptr;
	ime_handler_ = new client::OsrImeHandlerWin(hwnd_);

	::SetWindowLongPtr(hwnd_, GWLP_USERDATA, (LONG_PTR)this);
	::SetWindowLongPtr(hwnd_, GWLP_WNDPROC, (LONG_PTR)InteractionViewProc);
}

void InteractionViewWin::ResetInteraction()
{
	last_mouse_pos_ = {0, 0};
...
void InteractionViewWin::OnImeCompositionRangeChanged(
...
void InteractionViewWin::OnDestroy()
{
	source_cx_ = 0;
	source_cy_ = 0;
	browser_ = nullptr;
...
void InteractionViewWin::OnPaint()
{
	PAINTSTRUCT ps;
	BeginPaint(hwnd_, &ps);
...
POINT InteractionViewWin::TransMousePointToCef(const POINT &client_point)
{
	RECT rc;
	GetClientRect(hwnd_, &rc);
...
void InteractionViewWin::OnMouseEvent(UINT message, WPARAM wParam,
				      LPARAM lParam)
{
...
void InteractionViewWin::OnFocus(bool setFocus)
{
	if (browser_)
...
void InteractionViewWin::OnCaptureLost()
{
	if (mouse_rotation_)
		return;
...
void InteractionViewWin::OnKeyEvent(UINT message, WPARAM wParam, LPARAM lParam)
{
	if (!browser_)
		return;
...
void InteractionViewWin::OnIMESetContext(UINT message, WPARAM wParam,
					 LPARAM lParam)
{
...
void InteractionViewWin::OnIMEStartComposition()
{
	if (ime_handler_) {
		ime_handler_->CreateImeWindow();
		ime_handler_->MoveImeWindow();
...
void InteractionViewWin::OnIMEComposition(UINT message, WPARAM wParam,
					  LPARAM lParam)
{
...
void InteractionViewWin::OnIMECancelCompositionEvent()
{
	if (browser_ && ime_handler_) {
...
int InteractionViewWin::GetCefKeyboardModifiers(WPARAM wparam, LPARAM lparam)
{
	int modifiers = 0;
...
int InteractionViewWin::GetCefMouseModifiers(WPARAM wparam)
{
	int modifiers = 0;
