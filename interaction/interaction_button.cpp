...
{
	cursor_normal = LoadCursor(NULL, IDC_ARROW);
	cursor_hover = LoadCursor(NULL, IDC_HAND);
}
...
void InteractionButton::Create(HWND hp, int id)
{
	...
	hWnd = ::CreateWindow(L"BUTTON", L"",
			      ...
			      hp, NULL, GetModuleHandle(NULL), 0);
...
void InteractionButton::UpdateCursor(InteractionButtonState state)
{
	if (::IsWindow(hWnd)) {
		HCURSOR cursor;
		if (state == InteractionButtonState::ButtonHover ||
		    state == InteractionButtonState::ButtonClick) {
			cursor = cursor_hover;
		} else {
