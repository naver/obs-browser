...
//-------------------------------------------------------
std::vector<std::string> BrowserInteractionMainWin::font_list = {
	"Segoe UI", "MalgunGothic", "Malgun Gothic", "Dotum", "Gulin"};
HWND BrowserInteractionMainWin::prism_main = NULL;
HICON BrowserInteractionMainWin::interaction_icon = NULL;

void BrowserInteractionMainWin::SetGlobalParam(obs_data_t *private_data)
{
	...
	if (::IsWindow(hWnd)) {
		BrowserInteractionMainWin::prism_main = hWnd;
	} else {
...
void BrowserInteractionMainWin::RegisterClassName()
{
...
void BrowserInteractionMainWin::UnregisterClassName()
{
...
LRESULT CALLBACK BrowserInteractionMainWin::InteractionMainProc(HWND hWnd,
								UINT message,
								WPARAM wParam,
								LPARAM lParam)
{
	...
	BrowserInteractionMainWin *self =
		GetUserDataPtr<BrowserInteractionMainWin *>(hWnd);
...
//------------------------------------------------------------------------
BrowserInteractionMainWin::BrowserInteractionMainWin(SOURCE_HANDLE src)
	: BrowserInteractionMain(src), view_wnd(*this)
{
	view_wnd.ResetInteraction();
}

BrowserInteractionMainWin::~BrowserInteractionMainWin()
{
...
void BrowserInteractionMainWin::CreateInteractionUI()
{
...
void BrowserInteractionMainWin::WaitDestroyInteractionUI()
{
	DCHECK(CefCurrentlyOn(TID_UI));
...
void BrowserInteractionMainWin::ShowInteractionUI(bool show)
{
	DCHECK(CefCurrentlyOn(TID_UI));
...
void BrowserInteractionMainWin::BringUIToTop()
{
	DCHECK(CefCurrentlyOn(TID_UI));
...
void BrowserInteractionMainWin::SetInteractionInfo(
...
void BrowserInteractionMainWin::GetInteractionInfo(
...
void BrowserInteractionMainWin::OnImeCompositionRangeChanged(
...
bool BrowserInteractionMainWin::GetScreenPoint(CefRefPtr<CefBrowser> browser,
					       int viewX, int viewY,
					       int &screenX, int &screenY)
{
	auto hWnd = GetInteractionView();
	if (::IsWindow(hWnd)) {
		int source_cx_;
		int source_cy_;
		CefRefPtr<CefBrowser> cefBrowser;
		GetInteractionInfo(source_cx_, source_cy_,
						cefBrowser);
		...
		POINT screen_pt = {static_cast<LONG>(clientX),
				   static_cast<LONG>(clientY)};
...
bool BrowserInteractionMainWin::OnCursorChange(
...
void BrowserInteractionMainWin::PostWindowTitle(const char *str)
{
...
void BrowserInteractionMainWin::PostDestroyInteractionUI()
{
	should_close = true;
	if (::IsWindow(hWnd)) {
...
WindowHandle BrowserInteractionMainWin::GetInteractionView()
{
	if (window_created) {
...
WindowHandle BrowserInteractionMainWin::GetInteractionMain()
{
	if (window_created) {
		return hWnd;
	} else {
...
bool BrowserInteractionMainWin::IsResizing()
{
...
void BrowserInteractionMainWin::OnDestroy()
{
	hWnd = NULL;
...
void BrowserInteractionMainWin::OnPaint()
{
	PAINTSTRUCT ps;
	BeginPaint(hWnd, &ps);
...
void BrowserInteractionMainWin::OnSizing()
{
	RECT rc;
	GetClientRect(hWnd, &rc);
...
void BrowserInteractionMainWin::OnSize()
{
	SaveUserSize();
	UpdateLayout();
}

void BrowserInteractionMainWin::OnGetMinMaxInfo(LPARAM lParam)
{
	float dpi = GetWindowScaleFactor(hWnd);
...
void BrowserInteractionMainWin::OnDpiChanged(WPARAM wParam, LPARAM lParam)
{
...
LRESULT BrowserInteractionMainWin::OnNCHitTest(HWND hWnd, UINT message,
...
LRESULT BrowserInteractionMainWin::OnSetCursor(HWND hWnd, UINT message,
					       WPARAM wParam, LPARAM lParam)
{
...
LRESULT BrowserInteractionMainWin::OnNcLButtonDown(HWND hWnd, UINT message,
						   WPARAM wParam, LPARAM lParam)
{
...
bool BrowserInteractionMainWin::OnLButtonClick(UINT id, HWND hWnd)
{
	switch (id) {
...
void BrowserInteractionMainWin::OnSetTitle(WPARAM wParam, LPARAM lParam)
{
	...
	::SetWindowTextW(hWnd,
			 TransUtf8ToUnicode(m_utf8_title.c_str()).c_str());
}

void BrowserInteractionMainWin::OnCloseButton()
{
	// Handle this request in BrowserSource::Tick()
	InteractionManager::Instance()->RequestHideInteraction(m_source);
}

bool BrowserInteractionMainWin::ShouldClose()
{
	return should_close;
}

void BrowserInteractionMainWin::SaveUserSize()
{
...
void BrowserInteractionMainWin::UpdateLayout()
{
...
void BrowserInteractionMainWin::UpdateImage(float dpi)
{
...
void BrowserInteractionMainWin::ShowUI()
{
...
void BrowserInteractionMainWin::MovePosition()
{
	RECT prc;
...
void BrowserInteractionMainWin::DrawTitle(HDC &hdc, RECT &rc, float &dpi)
{
...
void BrowserInteractionMainWin::SetWindowSize(int left, int top, float dpi)
{
