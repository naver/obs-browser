	...
	: source(source_)
{
	...
	if (!obs_obj_is_private(source))
		interaction_delegate.reset(new InteractionDelegate(this));
	//PRISM/Renjinbo/20230518/shutdown will crash when have privatewebsource
	obs_frontend_add_event_callback(onPrismAppQuit_BrowserSource, this);
}
					 ...
					 void *context)
{
	...
	if (event == OBS_FRONTEND_EVENT_SCRIPTING_SHUTDOWN) {
		BrowserSource *view = static_cast<BrowserSource *>(context);
		if (view && view->GetBrowser()) {
			ActuallyCloseBrowser(view->GetBrowser());
			view->SetBrowser(nullptr);
		}
	}
#endif
}
...
void BrowserSource::Destroy()
{
	...
	if (interaction_delegate)
		interaction_delegate->DestroyInteraction();

	destroying = true;
	DestroyTextures();
	//PRISM/Renjinbo/20230518/shutdown will crash when have privatewebsource
	obs_frontend_remove_event_callback(onPrismAppQuit_BrowserSource, this);
...
bool BrowserSource::CreateBrowser()
{
	return QueueCEFTask([this]() {
		...
		//PRISM/Xiewei/20221226/obs upgrade: cef interaction
		if (interaction_delegate) {
			interaction_delegate->CreateInteractionUI();
...
void BrowserSource::dispatchJs(const char *eventName, const char *jsonData)
{
	DispatchJSEvent(eventName, jsonData, this);
}
...
void BrowserSource::Tick()
{
	...
	//PRISM/Xiewei/20221226/obs upgrade: cef interaction
	if (interaction_delegate)
		interaction_delegate->CheckInteractionShowing();
}
...
static void ExecuteOnOneBrowser(obs_source_t *source, BrowserFunc func)
{
	lock_guard<mutex> lock(browser_list_mutex);
...
{
	pls_source_send_notify(source, OBS_SOURCE_BROWSER_LOADED, 0);
}
